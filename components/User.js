...
const User = ({ item }) => {
  ...
  const sendFollow = async (currentUserId, selectedUserId) => {
    try {
      const response = await fetch("https://waste-recycle-app-backend.onrender.com/follow", {
        method: "POST",
        headers: {
  ...
  const handleUnfollow = async (targetId) => {
    try {
      const response = await fetch("https://waste-recycle-app-backend.onrender.com/users/unfollow", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
